...
void DroneStatus::setCurrectPosture(geometry_msgs::PoseStamped Posture) { this->currect_posture = Posture; }
...
uint8_t DroneStatus::detect_the_obscale(){
    ...
    for (int ii = dx_x - expand_range; ii < dx_x + expand_range; ii++) {
        if (ii < 0 || ii >= GLX_SIZE) break;
        for (int jj = dx_y - expand_range; jj < dx_y + expand_range; jj++) {
            if (jj < 0 || jj >= GLY_SIZE) break;
            for (int kk = dx_z - expand_range; kk < dx_z + expand_range; kk++) {
...
void DroneStatus::execute_the_route(){
    ...
    tf::Quaternion drone_pos;
    drone_pos.setW(currect_posture.pose.orientation.w);
    drone_pos.setX(currect_posture.pose.orientation.x);
    drone_pos.setY(currect_posture.pose.orientation.y);
    drone_pos.setZ(currect_posture.pose.orientation.z);
    double current_yaw = drone_pos.getAxis().getZ();

    if (theRoute.size() >= 3){
        // Enable A
        // 取出第一个位置点 和第二个位置点 计算角度差异
        Eigen::Vector3d att1 = theRoute[0],
         att2 = theRoute[1], att3 = theRoute[2];
        ...
        if (norm_v < velocity_drone && norm_a < acc_drone && norm_v > 1e-6) {
            this->expected_posture.type_mask = 0B101000000000;
            ...
            this->expected_posture.yaw = expected_yaw;
            ...
            this->expected_posture.type_mask = 0B101111111000;
        ...
        this->expected_posture.position.y = theRoute[0](1);
        this->expected_posture.position.z = theRoute[0](2);
        this->expected_posture.yaw = atan2(theRoute[0](1) - yy, theRoute[0](0) - xx);
    }
}
